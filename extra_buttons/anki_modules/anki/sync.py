...
print "WE'RE USING THE SYNC VERSION FROM EXTRA BUTTONS"
...
class Syncer(object):
    ...
    def sanityCheck(self):
        ...
        for t in "cards", "notes", "markdown", "revlog", "graves":
    ...
    def prepareToChunk(self):
        self.tablesLeft = ["revlog", "cards", "notes", "markdown"]
        self.cursor = None
...
from notes where %s""" % d)
            ...
            return x("""
select id, isconverted, md, html, mod, %d
from markdown where %s""" % d)
    ...
    def applyChunk(self, chunk):
        ...
        if "markdown" in chunk:
            self.mergeMarkdown(chunk['markdown'])
